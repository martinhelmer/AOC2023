...
Game 100: 3 red, 3 blue, 10 green; 3 green, 1 blue, 6 red; 5 red, 4 green, 7 blue""".strip().splitlines()
...
def parsed_line(line):
    ...
    grabs = [parse_grab(g) for g in cubes.split(";")]
    return gamenum, grabs 
...
def maxcolors(grabs):
    ...
    maxblue = 0
    for g in grabs:
        maxred = max(maxred, g[RED])
        maxgreen = max(maxgreen, g["green"])
        maxblue = max(maxblue, g["blue"])
    return {RED:maxred, "green":maxgreen, "blue":maxblue}

def ispossible(maxes):
    return maxes[RED] <= 12 and maxes[GREEN] <= 13 and maxes[BLUE] < 14
def p1(line):
    gn, p = parsed_line(line)
    return gn, maxcolors(p)

part1_answer = 2512
def part1(input):
    possible_games = [g for g,m in [p1(line) for line in input] if ispossible(m)]
